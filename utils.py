...
from yenchenlin.run_nerf_helpers import *
...
def get_batch(rays_rgb, i_batch, N_rand):
    ...
    if i_batch >= rays_rgb.shape[0]:
        ...
        i_batch = 0
    return i_batch, batch_rays, target_s

def train_preprocess(args, images, poses, bds, render_poses, i_test):
    hwf = poses[0,:3,-1]
    poses = poses[:,:3,:4]
...
def get_Ntrain_data_nums(args, poses):
    used_sample_num = int(poses.shape[0]*args.using_sample_percentage)
    if args.baseline:
        return [used_sample_num]
    v = (used_sample_num - args.n0) // args.n
    return [args.n]*v + [used_sample_num - args.n0- args.n*v]


def update_lr(args, optimizer, global_step):
...
def logs(args, i, render_poses, images, global_step, render_kwargs_train, optimizer, render_kwargs_test, 
         hwf, K, poses, loss, psnr, i_test):
    basedir = args.basedir
    expname = args.expname
    ...
    if i%args.i_video==0 and i > 0:
        ...
        with torch.no_grad():
            render_poses = render_poses[:2, :, :]  # TODO: test iteration
...
def config_parser():
    ...
    parser.add_argument("--n_iters",   type=int, default=50000, 
                        help='train iteration number')
    parser.add_argument("--n",   type=int, default=50000, 
                        help='iteration train sample number')
